...
if, for any initial condition $(x_0,\theta_0)$ and each detected CSI $\hat{s}_{\mu}$, there exists a gain $\hat{K}_{(\infty,\hat{s}_{\mu})}$, such that  $\hat{u}_{k}=\hat{K}_{(\infty,\hat{\theta}_{k-1})}x_k$ is a stabilizing SF control message. % for \eqref{eq:state}.
\end{definition}
...
\mathit{\Lambda} \triangleq \left( \bigoplus\nolimits_{\mathrm{j}=1}^{\mathrm{L}} 
    ...
    % \begin{bmatrix}
    %     \left( \mathcal{L}_{\mathrm{j}} \otimes \mathcal{L}_{\mathrm{j}} \right)\varrho_{{\mathrm{j}\mathrm{i}}}(\infty)
    % \end{bmatrix}_{\mathrm{i},\mathrm{j}=1}^{\mathrm{L}}
\end{equation}
